...
// Modified by Hongjian Yang, http://hacker-yhj.github.io/
// Did something about a kind of pop-out select window
(function() {
  d3.parsets = function() {
...
//--------------------------Done by Hongjian Y.---------------------------------
          //dimension.select("text").select("tspan.zoom.out")
              //.on("click.parsets", null );
//--------------------------Done by Hongjian Y.---------------------------------
          dimension.transition().duration(duration)
              .attr("transform", function(d) { return "translate(0," + d.y + ")"; })
              .tween("ribbon", ribbonTweenY);
        ...
        function updateCategories(g) {
          ...
          var categoryEnter = category.enter().append("g")
              .attr("class", "category")
              .attr("total", function(d){ return d.count; })
          ...
          category
              ...
              .on("mousedown.parsets", cancelEvent)
//--------------------------Done by Hongjian Y.---------------------------------
              .on("dblclick.parsets", clickCateEvent)
//--------------------------Done by Hongjian Y.---------------------------------
              .call(d3.behavior.drag()
                .origin(identity)
                .on("dragstart", function(d) {
...
//--------------------------Done by Hongjian Y.---------------------------------
    ...
    var tooltip = body.select(".detailed").append("div")
        .style("display", "none")
...
//--------------------------Done by Hongjian Y.---------------------------------
  function clickCateEvent() {
    ...
    { css: {
      ...
      message: "<p>You've selected <sp class='name'>" + $(t).children("text").attr("name")
        + "</sp> From <sp class='camp'>" + $(t).parent().find("text.dimension tspan.name").text() + "</sp></p>"
        + "<p>You can <button class='button button-pill button-flat button-tiny' onclick='zoom_in_handler();'>zoom into it</button> or <button class='button button-pill button-flat button-tiny' onclick='select_handler();'>Focus on it</button>.</p>"
        + "<p>Click the black overlay to cancel.</p>"
    });
